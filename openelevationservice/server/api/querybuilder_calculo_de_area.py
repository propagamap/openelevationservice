...
def polygon_elevation(geometry, format_out, dataset):
    ...
    if geometry.geom_type == 'Polygon':
        ...
        consulta_sql = """
            ...
            SELECT 
                ST_AsText(ST_MakeLine(
                    ARRAY(
                        SELECT ST_MakePoint(
                            ST_X(geom), ST_Y(geom), ST_Value(rast, 1, geom)
                        )
                        FROM oes_cgiar, polygon_geom,
                        LATERAL ST_PixelAsCentroids(ST_Clip(rast, polygon_geom.polygon), 1) AS geom
        ...
        result_geom = session.execute(text(consulta_sql), {"wkt_poligono": geometry.wkt}).scalar()

        # Ajustar el resultado para que incluya 'Z' en el formato LINESTRING Z
        if result_geom:
            result_geom = result_geom.replace("LINESTRING(", "LINESTRING Z(")

        #print('resultados z',result_geom)
       ...
       #-----Area---------
        ...
        consulta_sql = """
            ...
            SELECT ST_Area(polygon) / 1000000 AS area_km2
            FROM polygon_geom;
        """

        # Ejecutar la consulta SQL con el WKT del polígono proporcionado
        result_area = session.execute(text(consulta_sql), {"wkt_poligono": geometry.wkt}).scalar()
        print('Area en km cuadrados: ',result_area )
    ...
    if result_geom == None:
        raise InvalidUsage(404, 4002,
                           'The requested geometry is outside the bounds of {}'.format(dataset))
    print('-----------')
    print('cooooon mejoras sql lllll')
