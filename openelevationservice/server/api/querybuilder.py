...
def procesar_feature(feature):
    ...
    geometry = json.loads(feature["geometry"])
    poligono = shape(geometry)
    return altura, poligono

# Agrupación usando multiprocessing.Pool
def agrupar_tilas_por_altura_multiprocessing(datos):
    with Pool(4) as pool:
        resultados = pool.map(procesar_feature, datos["features"])
    agrupaciones = defaultdict(list)
    for altura, poligono in resultados:
        agrupaciones[altura].append(poligono)
...
def agrupar_tilas_por_altura_paralelo_modificada(datos, num_procesos=12, chunk_size=5):
    ...
    entradas = [(altura, poligonos) for altura, poligonos in resultados_multiprocessing.items()]
