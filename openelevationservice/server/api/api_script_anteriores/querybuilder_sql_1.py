    ...
    if geometry.geom_type == 'Polygon':
        ...
        consulta_sql = """
            ...
            SELECT 
                ...
                ST_Value(rast, 1, geom) AS height
            FROM 
                oes_cgiar, polygon_geom,
                LATERAL ST_PixelAsCentroids(ST_Clip(rast, polygon_geom.polygon), 1) AS geom
            WHERE 
                ST_Intersects(rast, polygon_geom.polygon);
        """

        # Ejecutar la consulta SQL con el WKT del polígono proporcionado
        resultados = session.execute(text(consulta_sql), {"wkt_poligono": geometry.wkt})

        

        print('resultado sql',resultados.fetchall())
