...
def classify_elevation(features_collection, min_height, max_height, num_ranges=23, no_data_value=-9999):
    # Calcula el tamaño del intervalo para dividir el rango de alturas
    range_div = (max_height - min_height + 1) / num_ranges

    # Variable para almacenar los resultados con la clasificación de altura
    classified_features = {
        "type": "FeatureCollection",
        "features": []
    }

    # Procesa cada feature de la colección
    for feature in features_collection['features']:
        # Obtiene la altura base de cada polígono
        height = feature['properties']['heightBase']
        
        # Calcula el rango de color (colorRange)
        color_range = max(
            min(
        ...
        # Calcula la altura base para este colorRange
        if color_range == -1:
            height_base = no_data_value  # Valor especial para fuera de rango
        else:
            height_base = math.ceil(color_range * range_div + min_height)

        # Actualiza las propiedades del polígono con el rango de color y altura base
        classified_feature = {
            "type": "Feature",
        ...
        # Agrega el feature clasificado a la colección de resultados
...
def procesar_union(entrada):
    ...
    # Asegurarse de que el resultado sea una lista de polígonos
    ...
    # Crear nuevas features a partir de los polígonos unidos
    nuevas_features = []
...
def agrupar_tilas_por_altura_paralelo(datos, num_procesos=12, chunk_size=5):
    ...
    # Agrupar los polígonos por altura
    ...
    # Crear una lista de entradas para el procesamiento paralelo
    entradas = [(altura, poligonos) for altura, poligonos in agrupaciones.items()]
    
    # Usar un Pool de procesos para realizar la unión de polígonos en paralelo
    with Pool(num_procesos) as p:
        # Usar imap_unordered para mejorar el rendimiento y evitar bloqueos
        resultados = p.imap_unordered(procesar_union, entradas, chunksize=chunk_size)
        
        # Combinar todas las nuevas features generadas
    ...
    try:
        ...
        if row:
           ...
           #print(" ")
            ...
            # # Usar la versión sin paralelizada de la función para agrupar las tilas por altura
