...
def polygon_elevation_ref(geometry, format_out, dataset):
    # Asegurar que la geometría es un polígono
    if geometry.geom_type != 'Polygon':
    ...
    # Crea una subconsulta para el polígono con SRID asignado
    query_geom = session.query(
        func.ST_SetSRID(func.ST_PolygonFromText(geometry.wkt), 4326).label('geom')
    ).subquery().alias('pGeom')

    # Modelo de datos raster obtenido según el dataset
    Model = _getModel(dataset)

    # Consulta para obtener los centroides de píxeles del raster recortado
    ...
    # Formatea los resultados a puntos 3D
    point_col, height_col = format_PixelAsGeoms(result_pixels)

    # Crea puntos 3D y los ordena
    ...
    # Realiza la consulta final para obtener la geometría deseada en el formato especificado
    ...
    # Determina el formato de salida y ejecuta la consulta adecuada
    if format_out == 'geojson':
    ...
    # Obtén el resultado geométrico y maneja los casos donde no se encuentre la geometría
    result_geom = query_final.scalar()
    if result_geom is None:
        raise ValueError('The requested geometry is outside the bounds of {}'.format(dataset))

    return result_geom

#AAOR-->polygon_elevation refactorizada otra

def polygon_elevation_ref_1(geometry, format_out, dataset):
    if geometry.geom_type != 'Polygon':
        raise ValueError("Needs to be a Polygon, not {}!".format(geometry.geom_type))
    ...
    result_pixels = session.query(
        func.DISTINCT(func.ST_PixelAsCentroids(func.ST_Clip(Model.rast, query_geom.c.geom, NO_DATA_VALUE), 1, False))
    ).select_from(query_geom.join(Model, func.ST_Intersects(Model.rast, query_geom.c.geom))).all()

    point_col, height_col = format_PixelAsGeoms(result_pixels)

    raster_points3d = session.query(
        func.ST_SetSRID(func.ST_MakePoint(func.ST_X(point_col), func.ST_Y(point_col), height_col), 4326).label('geom')
    ).order_by(func.ST_X(point_col), func.ST_Y(point_col)).subquery().alias('raster3d')
    ...
    query_final = session.query(func.ST_AsGeoJson(func.ST_Collect(query_points3d.c.geom))) if format_out == 'geojson' else session.query(func.ST_AsText(func.ST_MakeLine(query_points3d.c.geom)))
    result_geom = query_final.scalar()
    if result_geom is None:
        raise ValueError('The requested geometry is outside the bounds of {}'.format(dataset))
