...
def polygon_elevation(geometry, format_out, dataset):
    ...
    if geometry.geom_type == 'Polygon':
        ...
        resultado = session.execute(text(consulta_sql), {"wkt_poligono": geometry.wkt}).scalar()

        # Ajustar el resultado para que incluya 'Z' en el formato LINESTRING Z
        if resultado:
            resultado = resultado.replace("LINESTRING(", "LINESTRING Z(")

        print('resultados z',resultado)
        ...
        print('resultado result_geom',result_geom)
        #AAOR
